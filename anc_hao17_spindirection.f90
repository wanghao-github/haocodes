...
!*************************************************                      
  ...
  !    call mpi_bcast(mag_wann_orbs_index2,mag_wann_num2,MPI_INTEGER,0,mpi_comm_world,ierr)
      ...
      call mpi_bcast(mag_field_x1,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y1,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z1,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      
      call mpi_bcast(mag_field_x2,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y2,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z2,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)

      call mpi_bcast(mag_field_x3,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y3,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z3,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)

      call mpi_bcast(mag_field_x4,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y4,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z4,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)

      call mpi_bcast(mag_field_x5,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y5,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z5,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)

      call mpi_bcast(mag_field_x6,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y6,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z6,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)

      call mpi_bcast(mag_field_x7,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y7,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z7,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)

      call mpi_bcast(mag_field_x8,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_y8,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
      call mpi_bcast(mag_field_z8,1,MPI_DOUBLE_PRECISION, 0,mpi_comm_world,ierr)
  ...
  300    continue 
       ...
       !do i=1, 2
         !do j=1, num_wann_2
         !   spin_sigma_x(num_wann_2*(i-1)+(2*j-1), num_wann_2*(i-1)+2*j)=1.0d0
         !   spin_sigma_x(num_wann_2*(i-1)+2*j, num_wann_2*(i-1)+(2*j-1))=1.0d0
         !   spin_sigma_y(num_wann_2*(i-1)+(2*j-1), num_wann_2*(i-1)+2*j)=-zi
         !   spin_sigma_y(num_wann_2*(i-1)+2*j, num_wann_2*(i-1)+(2*j-1))=zi
         !   spin_sigma_z(num_wann_2*(i-1)+(2*j-1), num_wann_2*(i-1)+(2*j-1))=1.0d0
         !   spin_sigma_z(num_wann_2*(i-1)+2*j, num_wann_2*(i-1)+2*j)=-1.0d0
         !enddo
      !enddo
       do j=1, num_wann_2
  ...
  !    select_atom2 = reshape(select_atom2,[num_wann,num_wann])
  !    do n = 1, num_wann
  !       write(*, '(108I2)') (select_atom2(n, m),m=1,n)
  !    end do
 ...
 !        if(.not.allocated(rspauli)) allocate(rspauli(num_wann,num_wann,3,rvecnum))              
  ...
  !    endif 
     ...
     ! allocate(pauli(num_wann,num_wann,3))
      ...
      ! allocate(spin_dir_y(num_wann))
      ! allocate(spin_dir_z(num_wann))
      ...
      knv3= Nk1*Nk2*Nk3
       ...
       do ik= 1+ irank, knv3, isize
         ...
         do ii=1,rvecnum 
            ...
            pauli(:,:,1)=pauli(:,:,1)+fac*rspauli_final(:,:,1,ii)
            pauli(:,:,2)=pauli(:,:,2)+fac*rspauli_final(:,:,2,ii)
            pauli(:,:,3)=pauli(:,:,3)+fac*rspauli_final(:,:,3,ii)
      enddo 
...
!         spin_sigma_z_comp = MATMUL(MATMUL(eigvecs_z,spin_sigma_z),conjg(transpose(eigvecs_z))
         ...
         !spin_sigma_y_comp = 0.0d0
         !spin_sigma_z_comp = 0.0d0
         !spin_sigma_temp = 0.0d0

         !spin_sigma_x_comp = MATMUL(MATMUL(eigvecs,pauli(:,:,1)),eigvecs_dag)
         !spin_sigma_y_comp = MATMUL(MATMUL(eigvecs,pauli(:,:,2)),eigvecs_dag)
         !spin_sigma_z_comp = MATMUL(MATMUL(eigvecs,pauli(:,:,3)),eigvecs_dag)
        
        ! call mat_mul(num_wann,spin_sigma_x,eigvecs,spin_sigma_temp)
        ! call mat_mul(num_wann,eigvecs_dag,spin_sigma_temp,spin_sigma_x_comp)
        ! call mat_mul(num_wann,spin_sigma_y(:,:),eigvecs,spin_sigma_temp)
        ! call mat_mul(num_wann,eigvecs_dag,spin_sigma_temp,spin_sigma_y_comp(:,:))
...
!               Omega_x_t(m)= Omega_x(m)*(-1.0/(1*300))*((eigvals(m)-mu)*fermi(eigvals(m)-mu, Beta_fake)+(kbT*log(1+exp(-(eigvals(m)-mu)/kbT))))
 !              Omega_y_t(m)= Omega_y(m)*(-1.0/(1*300))*((eigvals(m)-mu)*fermi(eigvals(m)-mu, Beta_fake)+(kbT*log(1+exp(-(eigvals(m)-mu)/kbT))))
  !             Omega_z_t(m)= Omega_z(m)*(-1.0/(1*300))*((eigvals(m)-mu)*fermi(eigvals(m)-mu, Beta_fake)+(kbT*log(1+exp(-(eigvals(m)-mu)/kbT))))
              ...
              !  spin_sigma_x_t(m)=spin_sigma_x_comp(m,m)*fermi(eigvals(m)-mu, Beta_fake)
              !  spin_sigma_y_t(m)=spin_sigma_y_comp(m,m)*fermi(eigvals(m)-mu, Beta_fake)
              !  spin_sigma_z_t(m)=spin_sigma_z_comp(m,m)*fermi(eigvals(m)-mu, Beta_fake)
            ...
            enddo
              ...
              ! spin_sigma_t_mpi(1,step)= spin_sigma_t_mpi(1, step) + real(sum(spin_sigma_x_t))
              ! spin_sigma_t_mpi(2,step)= spin_sigma_t_mpi(2, step) + real(sum(spin_sigma_y_t))
              ! spin_sigma_t_mpi(3,step)= spin_sigma_t_mpi(3, step) + real(sum(spin_sigma_z_t))
...
!   call MPI_REDUCE(spin_sigma_t_mpi,spin_sigma_tmpi2,num_wann*3,MPI_DOUBLE_PRECISION,MPI_SUM,0,mpi_comm_world,ierr)
   ...
   !  spindirx = spin_dir_mpi2(1,:)
   !  spindiry = spin_dir_mpi2(2,:)
   !  spindirz = spin_dir_mpi2(3,:)
