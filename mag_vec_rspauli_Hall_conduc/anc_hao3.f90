...
!**************************************************************************                      
! Hao Wang modified base on a legacy code of Juelich research center 2022 Oct                                                                   
! mpiifort -CB -r8 -qmkl anc_hao3.f90 -o anc_hao3.x
!**************************************************************************                     
      implicit none 
  ...
  300    continue 
      ...
      !      do i=1,num_wann
      !        do j=1,num_wann
      !          hops(j,i,ii) = hops(j,i,ii) + mag_field_x * rspauli(j,i,1,ii) + mag_field_y * rspauli(j,i,2,ii) + mag_field_z * rspauli(j,i,3,ii)
      !        enddo !j
      !      enddo !i
      !    enddo
      !  endif
  ...
  500   continue 
     ...
     if(l_mag_vec)then
        do ii=1,rvecnum
          do i=1,num_wann
            do j=1,num_wann
              !if(ANY(i==mag_wann_orbs_index) .or. ANY(j==mag_wann_orbs_index)) then
                !print*, i,j
                hops(j,i,ii) = hops(j,i,ii) + mag_field_x * rspauli(j,i,1,ii) + mag_field_y * rspauli(j,i,2,ii) + mag_field_z * rspauli(j,i,3,ii)
              !endif
            enddo !j
     ...
     &           eigvecs(n1,n2)*conjg(eigvecs(n3,n4))                   
          ...
          enddo 
           ...
           Omega_x=0d0;Omega_y=0d0; Omega_z=0d0
            do m=1,num_wann
               do n=1,num_wann
                 if (abs(eigvals(m)-eigvals(n)) .gt. 0.00001d0) then 
      ...
      open(123,file='output_ahe_condquant',recl=10000) 
         do step=1,num_steps 
            ...
            write(123,*)"occupation=",occupation(step)                                
             !write(123,*)"conductivity=",conductivity_ahe(step)/(0.5*77.48e-6)*amat(3,3)  !2D direct convert to integer
             ...
             write(123,*)"conductivity=",conductivity_ahe(step)     /bohrincm   !unit is same with wannier_tool
             write(123,*)"conductivity13=",conductivity13_ahe(step) /bohrincm
             write(123,*)"conductivity23=",conductivity23_ahe(step) /bohrincm                      
            write(123,*)"************************************"                                                  
            enddo 
         close(123)                                                                                                                        
      endif 
